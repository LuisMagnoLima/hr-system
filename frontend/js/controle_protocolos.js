...
function render(){const tb=document.getElementById("cpTabela"),msg=document.getElementById("cpMensagem");tb.innerHTML="";if(!lista.length){msg.style.display="block";msg.textContent="Nenhum protocolo encontrado com os filtros informados.";return}msg.style.display="none";lista.forEach(p=>{const pend=p.status==="pendente_aceite",confirmado=p.status==="aceite_confirmado",cancelado=p.status==="cancelado",recusado=p.status==="recusado_destinatario",atrasado=pend&&p.dias_pendente>=2;const tr=document.createElement("tr");tr.innerHTML=`<td><b>${esc(p.protocolo)}</b><span class="cp-sub">Cadastrado por ${esc(p.remetente)}</span></td><td><b>${esc(p.numero_oficio)}</b></td><td>${esc(p.nome_documento)}<span class="cp-sub">${esc(p.interessado||"Sem interessado informado")}</span></td><td>${esc(p.secretaria)}<span class="cp-sub">${esc(p.setor_destino)}</span></td><td>${esc(p.destinatario)}</td><td><span class="cp-status ${pend?"pending":confirmado?"ok":recusado?"refused":"cancelled"}">${pend?"PENDENTE DE ACEITE":confirmado?"ACEITE CONFIRMADO":recusado?"RECUSADO PELO DESTINATÁRIO":"CANCELADO"}</span>${atrasado?`<span class="cp-status late">PENDENTE HÁ ${p.dias_pendente} DIAS</span>`:""}<span class="cp-sub">${recusado?`Recusado por ${esc(p.recusado_por||"-")}`:cancelado?`Cancelado por ${esc(p.cancelado_por||"-")}`:p.aceito_por?`Aceito por ${esc(p.aceito_por)}`:"Aguardando confirmação do setor"}</span></td><td>${dataBr(p.criado_em)}<span class="cp-sub">Aceite: ${dataBr(p.aceito_em)}</span></td><td class="cp-actions"><button class="cp-detail" onclick="detalhar('${p._id}')">Detalhes</button>${pend?`<button class="cp-edit" onclick="abrirEdicao('${p._id}')">Editar</button><button class="cp-cancel" onclick="cancelarProtocolo('${p._id}')">Cancelar</button>`:recusado?`<button class="cp-forward" onclick="abrirReencaminhamento('${p._id}')">Corrigir e Reencaminhar</button>`:""}<button class="cp-print" onclick="imprimir('${p._id}')">Imprimir</button></td>`;tb.appendChild(tr)})}
async function detalhar(id){try{const p=await apiFetch(`/controle-protocolos/${id}`);document.getElementById("cpDetalhes").innerHTML=`<h2>${esc(p.protocolo)}</h2><div class="cp-oficio">OFÍCIO ${esc(p.numero_oficio)}</div><p><b>Documento:</b> ${esc(p.nome_documento)}</p><p><b>Interessado:</b> ${esc(p.interessado||"Não informado")}</p><p><b>Secretaria / setor:</b> ${esc(p.secretaria)} / ${esc(p.setor_destino)}</p><p><b>Destinatário:</b> ${esc(p.destinatario)}</p><p><b>Observação:</b> ${esc(p.observacao||"Nenhuma")}</p>${p.status==="cancelado"?`<p><b>Justificativa do cancelamento:</b> ${esc(p.justificativa_cancelamento||"-")}</p>`:""}${p.status==="recusado_destinatario"?`<p><b>Recusado por:</b> ${esc(p.recusado_por||"-")}</p><p><b>Justificativa da recusa:</b> ${esc(p.justificativa_recusa||"-")}</p>`:""}<p><b>Reencaminhamentos:</b> ${esc(p.quantidade_reencaminhamentos||0)}</p><h3>Histórico do protocolo</h3><div class="cp-timeline">${(p.historico||[]).map(h=>`<div class="cp-event"><b>${esc(h.acao)}</b><div>${esc(h.usuario||"")}</div><small>${dataBr(h.data_hora)} • ${esc(h.situacao)}</small>${h.justificativa?`<p><b>Justificativa:</b> ${esc(h.justificativa)}</p>`:""}</div>`).join("")}</div>`;document.getElementById("cpModal").hidden=false}catch(e){alert(e.message)}}
